// app/search/page.js - Search Results Page for Filipino American Voices
'use client';
...
function SearchResultsContent() {
  ...
  const performSearch = async (searchQuery) => {
    ...
    try {
      const response = await fetch(
        `/api/search?q=${encodeURIComponent(searchQuery)}&site=tagalog&limit=50`
      );
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          setError('Nabigo ang paghahanap');
        }
      } else {
        setError('Nabigo ang kahilingan sa paghahanap');
      }
    } catch (error) {
      console.error('Search error:', error);
      setError('May error na naganap sa paghahanap');
    } finally {
      setLoading(false);
    }
  };
  ...
  // Category translations for Tagalog site
  const translateCategory = (category) => {
    const tagalogCategories = {
      'General': 'Pangkalahatan',
      'Philippines-US Relations': 'Relasyon Pilipinas-US',
      'China-US Relations': 'Balitang Internasyonal',
    ...
    return tagalogCategories[category] || category;
  };

  // Map category for FAV - China-US Relations appears as International News
  ...
  const getCategoryColor = (topic) => {
    ...
    switch(mappedTopic) {
      case 'Philippines-US Relations': return 'text-emerald-600 bg-emerald-50';
          ...
          <div className="flex items-center">
            ...
            <div className="overflow-hidden flex-1">
              <div className="text-sm">
                {query ? `Resulta ng paghahanap: "${query}"` : 'Mag-type ng keyword para maghanap ng balita'}
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-gray-50 border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex items-center py-4 space-x-8 overflow-x-auto">
            ...
            >
              <span>Lahat ng balita</span>
            </Link>
            {query && (
              <div className="flex items-center space-x-2 px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap bg-emerald-500 text-white">
                <Search className="w-4 h-4" />
                <span>Resulta ng paghahanap</span>
              </div>
            )}
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="mb-8">
          <nav className="flex items-center space-x-2 text-sm text-gray-600 mb-6">
            ...
            <span>›</span>
            <span className="text-gray-900">Resulta ng paghahanap</span>
          </nav>
            ...
            <div className="mb-4">
              ...
              {results.length > 0 && (
                <p className="text-gray-600">
                  Nahanap ang {results.length} kaugnay na artikulo
                </p>
              )}
            </div>
          )}
        ...
        {loading && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">Naghahanap...</p>
          </div>
        )}
        ...
        {!loading && !error && query && results.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 mb-2">Walang nahanap na kaugnay na artikulo</p>
            <p className="text-gray-500 text-sm">Subukan ang ibang keyword</p>
          </div>
        )}
        ...
        {!loading && results.length > 0 && (
          <div>
            <h2 className="text-xl font-bold text-gray-900 mb-6">Resulta ng paghahanap</h2>
                      ...
                      <div className="flex-1 p-4">
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            {/* Title */}
                            <h3 className="text-lg font-semibold text-gray-900 group-hover:text-emerald-600 transition-colors leading-tight mb-2">
        ...
        {!query && (
          <div className="text-center py-12">
            <Search className="w-12 h-12 text-gray-300 mx-auto mb-4" />
            <p className="text-gray-600">Mag-type ng keyword para maghanap</p>
          </div>
        )}
      </main>
      ...
      <footer className="bg-gray-900 text-white mt-16">
        <div className="max-w-7xl mx-auto px-6 py-12">
          ...
          <div className="border-t border-gray-800 mt-8 pt-8 text-center text-sm text-gray-400">
            <p>&copy; 2025 Tinig ng Filipino Amerikano. Lahat ng karapatan ay nakalaan.</p>
          </div>
        </div>
      </footer>
