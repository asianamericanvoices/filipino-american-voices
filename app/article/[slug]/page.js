// app/article/[slug]/page.js - Filipino American Voices Article Page
'use client';
...
export default function ArticlePage({ params }) {
  ...
  useEffect(() => {
    ...
    const fetchArticle = async () => {
      try {
        ...
        const response = await fetch(`/api/article/${articleId}?language=tagalog&t=${Date.now()}`);

        if (!response.ok) {
          if (response.status === 404) {
            console.error('Article not found for ID:', articleId);
            setError('Hindi mahanap ang artikulo');
            setLoading(false);
            return;
          }
        ...
        try {
          const relatedResponse = await fetch(`/api/published-articles?language=tagalog&category=${foundArticle.topic}&limit=4&t=${Date.now()}`);
        ...
        setError('May error sa pag-load ng artikulo');
        setLoading(false);
      }
    };
  ...
  const getCategoryNameInTagalog = (topic) => {
    const directCategoryMap = {
      'Event Explainers': 'Mga Paliwanag sa Pangyayari',
      'Philippines-US Relations': 'Relasyon Pilipinas-US',
      'International News': 'Balitang Internasyonal',
      'US Politics': 'Pulitika ng US',
  ...
  const getDisplayTitle = (article) => {
    return article.translatedTitles?.tagalog ||
  ...
  const getDisplaySummary = (article) => {
    return article.translations?.tagalog ||
           article.aiSummary ||
           'Wala pang salin sa Tagalog';
  };
      ...
      <div className="min-h-screen bg-white">
          ...
          <div className="max-w-7xl mx-auto px-6">
            <div className="flex items-center justify-between h-16">
              <Link href="/" className="flex items-center space-x-4">
                <div className="w-8 h-8 bg-emerald-500 rounded-lg flex flex-col items-center justify-center gap-0.5">
                  <span className="text-white font-bold text-[5px] leading-none">Tinig ng</span>
                  <span className="text-white font-bold text-[5px] leading-none">Filipino</span>
                  <span className="text-white font-bold text-[5px] leading-none">Amerikano</span>
                </div>
                <div>
        ...
        <div className="flex items-center justify-center min-h-[50vh]">
          <div className="text-center">
            ...
            <p className="text-gray-600">Naglo-load ng artikulo...</p>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-white">
        ...
        <div className="max-w-4xl mx-auto px-6 py-16 text-center">
          ...
          <p className="text-gray-600 mb-8">Paumanhin, hindi mahanap ang artikulong hinahanap mo.</p>
          <Link href="/" className="bg-emerald-500 text-white px-6 py-3 rounded-lg hover:bg-emerald-600 transition-colors">
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex items-center justify-between h-16">
            ...
            <nav className="hidden md:flex items-center space-x-8">
              <Link href="/" className="text-gray-700 hover:text-emerald-600 font-medium">Pinakabago</Link>
              <a href="#" className="text-gray-700 hover:text-emerald-600 font-medium">Pulitika</a>
              <a href="#" className="text-gray-700 hover:text-emerald-600 font-medium">Komunidad</a>
              <a href="#" className="text-gray-700 hover:text-emerald-600 font-medium">Negosyo</a>
              <a href="#" className="text-gray-700 hover:text-emerald-600 font-medium">Kultura</a>
            </nav>
          </div>
        </div>
      </header>
      ...
      <div className="bg-gray-50 py-4">
        <div className="max-w-4xl mx-auto px-6">
          <div className="flex items-center space-x-2 text-sm text-gray-600">
            ...
            <span>›</span>
            <span className="font-medium">{getCategoryNameInTagalog(article.topic)}</span>
            <span>›</span>
            <span className="text-gray-900">Detalye ng artikulo</span>
          </div>
        </div>
      </div>
      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        ...
        <article>
          ...
          <header className="mb-8">
            <div className="mb-4">
              <span className="inline-block px-3 py-1 bg-emerald-100 text-emerald-800 text-sm font-medium rounded-full">
                {getCategoryNameInTagalog(article.topic)}
              </span>
            </div>

            <h1 className="text-4xl lg:text-5xl font-bold text-gray-900 leading-tight mb-6 font-filipino">
                  ...
                  {article.dateline && (
                    <>
                      <span className="font-medium text-gray-900">
                        Balita mula sa {article.dateline}
                      </span>
                      <span>•</span>
                    </>
                  )}
                  ...
                  {article.author && article.author !== 'N/A' && article.author !== 'Unknown' && article.author !== 'Staff' && (
                    <>
                      ...
                      <span className="text-gray-600">
                        Orihinal na may-akda: {article.author}
                      </span>
                    </>
                  )}
                </div>
            ...
            <div className="flex items-center justify-between mb-6">
              <HeartButton articleId={article.id} site="tagalog" />
              ...
              <div className="flex items-center space-x-4">
                ...
                <button
                  ...
                  title="Kopyahin ang link"
                >
                  <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 16H8V7h11v14z"/>
                  </svg>
                  <span className="text-sm hidden md:inline">Kopyahin</span>
                </button>
              </div>
            </div>
            ...
            {/* Audio Player */}
            <ArticleAudioPlayer
              audioUrl={article.tagalogAudioUrl || article.audioUrl}
              title={getDisplayTitle(article)}
              duration={article.tagalogAudioDuration || article.audioDuration}
              language="tagalog"
            />
          </header>
          ...
          {article.imageUrl && (
            <div className="mb-8">
              ...
              />
              <p className="text-xs text-gray-500 italic mt-2 text-center font-filipino">
                {article.socialCaptions?.tagalog && (
                  <span className="block mb-1">{article.socialCaptions.tagalog}</span>
                )}
                <span>
                  {article.imageAttribution ? (
                    <>
                      ...
                      {article.imageAttribution.source === 'unsplash' ? (
                        <a
                          href="https://unsplash.com?utm_source=filipino-american-voices&utm_medium=referral"
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-emerald-600 hover:text-emerald-800 underline"
                        >
                        ...
                        article.imageAttribution.source || 'Pinagmulan ng larawan'
                      )}
                    </>
                  ) : (
                    'Hindi kilalang pinagmulan ng larawan'
                  )}
                </span>
              </p>
            </div>
          ...
          <div className="prose prose-lg max-w-none mb-8">
            <div className="text-gray-700 font-filipino leading-relaxed text-lg">
              <div
          ...
          <div className="bg-emerald-50 p-6 rounded-lg mb-8">
            <h3 className="text-lg font-bold text-gray-900 mb-3 font-filipino">Basahin ang orihinal</h3>
            <p className="text-gray-700 mb-4 font-filipino">
              I-click ang link sa ibaba para basahin ang buong orihinal na artikulo mula sa {article.source}.
            </p>
            <a
            ...
            >
              <span>Bisitahin ang {article.source}</span>
        ...
        {relatedArticles.length > 0 && (
          <div className="mt-16 pt-8 border-t border-gray-200">
            <h2 className="text-2xl font-bold text-gray-900 mb-8 font-filipino">Kaugnay na Balita</h2>
              ...
              {relatedArticles.map((relatedArticle) => (
                ...
                >
                  <div className="bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition-shadow">
                    ...
                    />
                    <div className="p-4">
                      <h3 className="font-semibold text-gray-900 group-hover:text-emerald-600 transition-colors line-clamp-2 font-filipino">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="md:col-span-2">
              <div className="flex items-center space-x-3 mb-4">
                <div className="w-12 h-12 bg-emerald-500 rounded-lg flex flex-col items-center justify-center gap-0.5">
                  <span className="text-white font-bold text-[10px] leading-none tracking-wide">Tinig</span>
                  <span className="text-white font-bold text-[10px] leading-none tracking-wide">FAV</span>
                </div>
                <div>
              ...
              <p className="text-gray-400 text-sm leading-relaxed font-filipino">
                Naghahatid ng mahahalagang balita para sa komunidad ng Filipino American. Kami ay isang independiyenteng media na nakatuon sa mga isyung may epekto sa ating komunidad.
              </p>
            </div>
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm">
                <li><Link href="/about" className="text-gray-400 hover:text-white font-filipino">Aming misyon</Link></li>
