...
const SITE = 'https://www.tinigngfilipinoamerikano.us';
...
const getArticle = cache(async (slug) => {
  try {
    const res = await fetch(`${FETCH_BASE}/api/article/${slug}?language=tagalog`, {
      cache: 'no-store',
    });
...
export async function generateMetadata({ params }) {
  ...
  const description = stripHtml(tagalogSummary).substring(0, 160) + '...';
  const imageUrl = article.imageUrl || '/og-logo-filipino-3.png';
  ...
  return {
    ...
    openGraph: {
      ...
      images: [{ url: imageUrl, width: 1200, height: 630, alt: tagalogTitle }],
    },
    twitter: {
...
function newsArticleLd(article, slug) {
  const headline = stripHtml(article.translatedTitles?.tagalog || article.originalTitle);
  const authorName =
    article.author && !['N/A', 'Unknown', 'Staff'].includes(article.author)
      ? article.author
      : article.source || 'Tinig ng Filipino Amerikano';
  return {
    ...
    publisher: {
      ...
      logo: {
        '@type': 'ImageObject',
        url: `${SITE}/og-logo-filipino-3.png`,
      },
    },
