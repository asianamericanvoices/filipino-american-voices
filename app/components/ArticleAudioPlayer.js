...
export default function ArticleAudioPlayer({ audioUrl, title, duration, language = 'tagalog' }) {
  ...
  const texts = {
    ...
    tagalog: {
      listen: 'Pakinggan ang artikulong ito',
      loading: 'Naglo-load...',
      error: 'Hindi mapatugtog ang audio',
      speed: 'Bilis'
    },
    english: {
  ...
  const t = texts[language] || texts.tagalog;

  // Ensure we're rendering something
  console.log('✅ ArticleAudioPlayer rendering with audioUrl:', audioUrl);

  return (
    <div className="bg-emerald-50 rounded-lg p-4 mb-6 border border-emerald-200" data-audio-player="true">
      <audio
        ref={audioRef}
        src={audioUrl}
        preload="metadata"
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-medium text-emerald-800">{t.listen}</span>
      ...
      {hasError ? (
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <SkipBack className="w-4 h-4 text-emerald-600" />
          </button>
          ...
          >
            <SkipForward className="w-4 h-4 text-emerald-600" />
          </button>

          {/* Progress bar - clickable for seeking */}
          <div className="flex-1 mx-2">
            <div
              className="h-2 bg-emerald-100 rounded-full overflow-hidden cursor-pointer relative"
              onClick={(e) => {
            ...
            >
              <div
                className="h-full bg-emerald-500 transition-all duration-200 pointer-events-none"
                style={{
      ...
      <div className="mt-3 text-xs text-gray-600 italic text-center">
        {language === 'korean' ? (
          ...
          <span>Ang audio na ito ay binasa ng AI voice na pinapagana ng ElevenLabs</span>
        ) : (
