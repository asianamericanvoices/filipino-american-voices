...
const SearchBar = ({ className = '', site = 'tagalog' }) => {
  ...
  const getDisplayTitle = (article) => {
    if (site === 'tagalog' && article.translatedTitles?.tagalog) {
  ...
  const highlightMatch = (text, searchQuery) => {
    ...
    try {
      ...
      return text.replace(regex, '<mark class="bg-emerald-200">$1</mark>');
    } catch (e) {
      return text;
    }
  };
    ...
    <div className={`relative ${className}`} ref={searchRef}>
      ...
      {!isOpen ? (
        <button
          ...
          aria-label="Maghanap ng mga artikulo"
        >
          <Search className="w-5 h-5" />
          <span className="hidden md:inline text-sm">Maghanap</span>
        </button>
      ) : (
        /* Expanded Search */
        <div className="fixed right-4 top-16 w-80 sm:w-96 sm:absolute sm:right-0 sm:top-0 max-w-[calc(100vw-2rem)] sm:max-w-none bg-white border border-gray-200 rounded-lg shadow-lg z-50">
          ...
          <div className="flex items-center p-4 border-b border-gray-100">
            ...
            <input
              ...
              placeholder="Maghanap ng mga artikulo..."
          ...
          <div className="max-h-96 overflow-y-auto">
            {loading && (
              <div className="p-4 text-center text-gray-500 text-sm">
                <div className="animate-pulse">Naghahanap...</div>
              </div>
            )}

            {!loading && query && results.length === 0 && (
              <div className="p-4 text-center text-gray-500 text-sm">
                Walang nahanap na kaugnay na artikulo
              </div>
            )}
            ...
            {!loading && query && results.length > 0 && (
              <>
                <div className="px-4 py-2 text-xs text-gray-500 bg-gray-50">
                  Nahanap ang {results.length} artikulo
                </div>
                {results.map((article) => (
                  <Link
                    key={article.id}
                ...
                {results.length >= 6 && (
                  <button
                    ...
                    className="w-full p-3 text-sm text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50 border-t border-gray-100"
                  >
            ...
            {!query && recentSearches.length > 0 && (
              <div className="p-4">
                <div className="flex items-center justify-between mb-3">
                  <h4 className="text-sm font-medium text-gray-900">Kamakailang hinanap</h4>
                  <button
            ...
            {!query && recentSearches.length === 0 && (
              <div className="p-4 text-center text-gray-500 text-sm">
                Mag-type ng keyword para maghanap
              </div>
            )}
          </div>
        </div>
