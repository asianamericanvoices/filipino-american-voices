...
export default function ArticleRequestForm() {
  ...
  const translations = {
    title: "Humiling ng pagsasalin ng artikulo",
    urlLabel: "Link ng artikulo",
    urlPlaceholder: "I-paste ang link ng artikulong gusto mong isalin",
    nameLabel: "Pangalan",
    namePlaceholder: "Iyong pangalan",
    emailLabel: "Email",
    emailPlaceholder: "Iyong email address",
    zipLabel: "Zip code (opsyonal)",
    zipPlaceholder: "Iyong zip code",
    commentLabel: "Komento (opsyonal)",
    commentPlaceholder: "Bakit gusto mong isalin ang artikulong ito?",
    submitButton: "Ipadala ang kahilingan",
    submittingButton: "Ipinapadala...",
    successMessage: "Salamat sa iyong kahilingan! Susuriin ito ng aming editorial team.",
    errorMessage: "Nabigo ang pagpapadala. Pakisubukan muli mamaya.",
    rateLimitMessage: "Masyadong maraming kahilingan ngayon. Pakisubukan bukas.",
  ...
  const validateForm = () => {
    ...
    if (!formData.articleUrl || !formData.name || !formData.email) {
      setMessage({ type: 'error', text: 'Pakipunan ang lahat ng kinakailangang field.' });
      return false;
    }
    ...
    if (!emailRegex.test(formData.email)) {
      setMessage({ type: 'error', text: 'Pakilagay ang wastong email address.' });
      return false;
    }
    ...
    try {
      ...
      setMessage({ type: 'error', text: 'Pakilagay ang wastong URL.' });
      return false;
    }

    // Check CAPTCHA
    if (!captchaToken) {
      setMessage({ type: 'error', text: 'Pakikumpleto ang reCAPTCHA.' });
      return false;
    }
      ...
      <div className="bg-emerald-50 rounded-lg p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            ...
            className="w-full bg-emerald-500 text-white py-2 px-4 rounded-md hover:bg-emerald-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed font-medium text-sm"
          >
