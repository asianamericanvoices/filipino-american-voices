  ...
  try {
    ...
    if (!email || !token) {
      return NextResponse.json(
        { error: 'Kulang ang kinakailangang impormasyon', errorEn: 'Missing required parameters' },
        { status: 400 }
      );
    }
    ...
    if (!verifyToken(email, token)) {
      return NextResponse.json(
        { error: 'Hindi wastong unsubscribe link', errorEn: 'Invalid unsubscribe link' },
        { status: 403 }
      );
    }
    ...
    if (!supabase) {
      return NextResponse.json(
        { error: 'Pansamantalang hindi available ang serbisyo', errorEn: 'Service temporarily unavailable' },
        { status: 500 }
      );
    }
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: 'Hindi nagtagumpay ang pag-unsubscribe. Pakisubukan ulit mamaya', errorEn: 'Unsubscribe failed, please try again later' },
        { status: 500 }
      );
    }

    console.log(`✅ Unsubscribed: ${email} (Filipino site)`);

    return NextResponse.json({
      success: true,
      message: 'Na-unsubscribe ka na sa newsletter',
    ...
    console.error('Unsubscribe error:', error);
    return NextResponse.json(
      { error: 'Hindi nagtagumpay ang pag-unsubscribe', errorEn: 'Unsubscribe failed' },
      { status: 500 }
    );
  }
}
