// app/api/analytics/track/route.js - Analytics tracking endpoint for Filipino American Voices
...
function hashIP(ip) {
  if (!ip) return null;
  return crypto.createHash('sha256').update(ip + process.env.IP_SALT || 'filipino-american-voices').digest('hex').substring(0, 16);
}
...
async function handleReadingStart(metadata) {
  try {
    const readingData = {
      ...
      language: metadata.article_language || 'tl'
    };
